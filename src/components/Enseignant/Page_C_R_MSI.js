...
function Page_MSI() {
  ...
  useEffect(() => {
    ...
    Axios.get(`http://localhost:3002/session/msi/is_open`)
      .then((res) => {
        if (res.status === 200) {
          setIsMSISessionOpened(res.data.is_open);
        } else {
  ...
  const handleSubmit = (e) => {
    e.preventDefault();
    Axios.post(`http://localhost:3002/${Username}/recours_MSI`, {
  ...
  const handleAnnuler = async () => {
    try {
      const response = await fetch(`/msi/${Username}/cancel`, {
        method: "PUT",
        headers: {
  ...
  const handleCheck = async () => {
    try {
      const response = await fetch(`/msi/${Username}/decision`, {
        method: "GET",
        headers: {
          "Content-Type": "application/json",
        },
    ...
    >
      ...
      >
        ...
        >
          ...
          ></i>
          {isMSISessionOpened ? (
            <Link to={`/Eseignant/MSI1/${Username}`}>
              <span className="nav-item">Plan your departure</span>
            </Link>
          ) : (
            <p>
              The session "Manifestation Scientifique Internationale"is not yet
              open
            </p>
          )}
        </div>
