...
const Traitement_Demande = () => {
  ...
  const handleSubmit = (e) => {
    e.preventDefault();
    Axios.put(`http://localhost:3002/SPE/update/${currentDoctorant.id}`, { note })
      .then((res) => {
        if (res.status === 200) {
  ...
  const columns = [
    { field: 'Username_Mat', headerName: 'Matricule', width: 100 },
    { field: 'Pays', headerName: 'Pays', width: 100 },
    { field: 'Ville', headerName: 'Ville', width: 100 },
    { field: 'Etablissement_acc', headerName: 'Etablissement', width: 150 },
    { field: 'Date_dep', headerName: 'Date de départ', width: 120 },
    { field: 'Date_retour', headerName: 'Date de retour', width: 120 },
